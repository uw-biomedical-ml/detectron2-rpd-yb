    ...
    def output_to_pdf(self,ImgIds,outname,dfimg=None):
        ...
        with PdfPages(outname) as pdf:
            for imgid in tqdm(ImgIds):
                ...
                pdf.savefig(fig)
                plt.close(fig)
    def output_masks_to_tiff(self, output_path, ImgIds, ptid, eye):
        imgs = []
        ...
        if not os.path.isdir(os.path.join(output_path,'binary_masks')):
            os.mkdir(os.path.join(output_path,'binary_masks'))
        if len(imgs) > 1:
            imgs[0].save(os.path.join(output_path, 'binary_masks', str(ptid) + "_" + str(eye) + "-bmasks.tif"), tags = "test", compression = "tiff_deflate", save_all=True, append_images=imgs[1:])
        else:
            imgs[0].save(os.path.join(output_path, 'binary_masks', str(ptid) + "_" + str(eye) + "-bmasks.png"))
    
    def output_overlay_masks_to_tiff(self, output_path, ImgIds, ptid, eye):
        imgs = []
        for index in range(len(ImgIds)):
            gt_data = next(item for item in self.data if (item['image_id'] == ImgIds[index]))   
            dat = gt_data
            ...
            pil_model = Image.fromarray(result_model)
            imgs.append(pil_model)
        if not os.path.isdir(os.path.join(output_path,'binary_masks_overlays')):
            os.mkdir(os.path.join(output_path,'binary_masks_overlays'))
        if len(imgs) > 1:
            imgs[0].save(os.path.join(output_path, "binary_masks_overlays", str(ptid) + "_" + str(eye) + "-bmasks_overlay.tif"), tags = "test", compression = "tiff_deflate", save_all=True, append_images=imgs[1:])
        else:
            imgs[0].save(os.path.join(output_path, "binary_masks_overlays", str(ptid) + "_" + str(eye) + "-bmasks_overlay.png"))

    def output_instances_masks_to_tiff(self, output_path, ImgIds, ptid, eye):
        imgs = []
        for index in range(len(ImgIds)):
            gt_data = next(item for item in self.data if (item['image_id'] == ImgIds[index]))   
            dat = gt_data
            ...
            pil_model = Image.fromarray(result_model)
            imgs.append(pil_model)
        if not os.path.isdir(os.path.join(output_path,'instance_mask_overlays')):
            os.mkdir(os.path.join(output_path,'instance_mask_overlays'))
        if len(imgs) > 1:
            imgs[0].save(os.path.join(output_path, "instance_mask_overlays", str(ptid) + "_" + str(eye) + "-ipmasks_overlay.tif"), tags = "test", compression = "tiff_deflate", save_all=True, append_images=imgs[1:])
        else:
            imgs[0].save(os.path.join(output_path, "instance_mask_overlays", str(ptid) + "_" + str(eye) + "-ipmasks_overlay.png"))
